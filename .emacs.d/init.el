...
(use-package auto-package-update
  ...
  ;; `package-install-from-archive' extracts .tar files into *tar-data *temp*
  ;; buffers that are not always killed, leaking memory over long sessions.
  ;; Reclaim them after every update run.
  (defun daut/cleanup-package-temp-buffers (&rest _)
    "Kill leftover *tar-data *temp* buffers leaked by package operations."
    (dolist (b (buffer-list))
      (let ((name (buffer-name b)))
        (when (and (string-prefix-p " *tar-data" name)
                   (string-match-p "temp" name))
          (when (buffer-live-p b)
            (kill-buffer b))))))
  (advice-add 'auto-package-update-now :after #'daut/cleanup-package-temp-buffers)
...
;; `Info-directory-list' only exists once info.el is loaded; adding to it
;; unconditionally aborts init when info hasn't loaded yet.
...
(use-package emacs
  ...
  :config
  ;; Prefer the Nerd Font variant (carries PUA glyphs on Omarchy); fall back
  ;; to plain JetBrains Mono where only that is installed (macOS).
  (defvar daut/default-font-family
    (or (seq-find (lambda (f) (member f (font-family-list)))
...
(use-package dashboard
  :ensure t
  :custom
  ;; Read the projects list from projectile (with search-path discovery and
  ;; ignore rules), not the default project-el backend whose list accumulates
  ;; ~/.emacs.d/elpa entries via package-vc upgrades.
...
(use-package nerd-icons
  :custom
  ;; Omarchy only has "JetBrainsMono Nerd Font"; macOS only has
  ;; "Symbols Nerd Font Mono". Pick whichever is installed.
...
(use-package projectile
  ...
  ;; Never treat anything under ~/.emacs.d as a project (elpa VC packages
  ;; would otherwise pollute the projects list, e.g. via package-vc upgrades).
  (setq projectile-ignored-project-function
        (lambda (project-root)
          (string-prefix-p (file-truename user-emacs-directory)
                           (file-truename project-root))))
  ;; Purge the entries the default project.el backend already remembered;
  ;; consult and other project.el consumers read the same list.
  ;; project-known-project-list was removed in Emacs 31; forget-under is the
  ;; purpose-built API (absent on older Emacs, hence the fboundp guard).
  (with-eval-after-load 'project
    (when (fboundp 'project-forget-projects-under)
      (project-forget-projects-under user-emacs-directory 'recursive)))
  (setq projectile-project-search-path
        (append (when (file-directory-p "~/projects") '(("~/projects" . 3)))
...
(use-package company-statistics
  ...
  :config
  ;; `company-statistics--save' rewrites the cache file without the
  ;; lexical-binding cookie, which makes `load' warn on every startup.
  (defun daut/company-statistics-save-with-cookie (orig-fn)
    "Add the lexical-binding cookie `company-statistics--save' omits."
    (funcall orig-fn)
    (when (and company-statistics-file (file-exists-p company-statistics-file))
      (with-temp-file company-statistics-file
        (insert ";;; -*- lexical-binding: t; -*-\n")
        (insert-file-contents company-statistics-file))))
  (advice-add 'company-statistics--save :around #'daut/company-statistics-save-with-cookie))
...
(defun daut/copilot--native-platform ()
  "Return the release platform string like \"linux-x64\" for this system."
  (let ((os (pcase system-type
        ...
        (arch (pcase system-configuration
                ((pred (string-match-p "aarch64")) "arm64")
                ((pred (string-match-p "x86_64")) "x64")
                (_ (error "Unsupported architecture: %s" system-configuration)))))
    (format "%s-%s" os arch)))

(defun daut/copilot--latest-server-url ()
  "Return the download URL of the latest native Copilot server release."
  (require 'json)
  (let ((buf (url-retrieve-synchronously
    ...
    (unwind-protect
        (with-current-buffer buf
          ...
          (let ((assets (alist-get 'assets (json-read))))
            (thread-last assets
              (seq-find (lambda (asset)
                          (string-match-p
                           (format "copilot-language-server-%s-[0-9.]+\\.zip"
                                   (daut/copilot--native-platform))
                           (alist-get 'name asset))))
              (alist-get 'browser_download_url))))
      (kill-buffer buf))))

(defun daut/copilot-install-server ()
  "Install the native Copilot language server from GitHub releases."
  (unless (executable-find "unzip")
    (error "Cannot install Copilot server: `unzip' is required"))
  (let* ((url (or (daut/copilot--latest-server-url)
                  (error "No Copilot server release found for %s"
                         (daut/copilot--native-platform))))
         (zip (make-temp-file "copilot-server" nil ".zip"))
         (tmpdir (make-temp-file "copilot-server" t))
         (bin (file-name-concat copilot-install-dir "bin" copilot-server-executable)))
    (message "Copilot: downloading %s..." url)
    (url-copy-file url zip t)
    (unwind-protect
        (progn
          (let ((exit (call-process "unzip" nil nil nil
                                    "-q" "-o" zip "-d" tmpdir "copilot-language-server")))
            (unless (eq exit 0)
              (error "Failed to extract Copilot server zip (exit code %s)" exit)))
          (make-directory (file-name-directory bin) 'parents)
          (copy-file (expand-file-name "copilot-language-server" tmpdir) bin t)
          (set-file-modes bin #o755)
          (message "Copilot: installed native server at %s" bin))
      (delete-file zip)
      (delete-directory tmpdir 'recursive))))

(defun daut/copilot-ensure-server ()
  "Install the Copilot language server if it is missing."
  (unless (or (executable-find copilot-server-executable)
              (file-exists-p
               (file-name-concat copilot-install-dir "bin" copilot-server-executable)))
    (condition-case err
        (daut/copilot-install-server)
      (error (message "Copilot: server install failed: %s"
